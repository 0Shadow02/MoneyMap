...
type TransactionHistoryRow = GetTransactionsHistoryResponseType[0]
 const columns: ColumnDef<TransactionHistoryRow>[] =[
    ...
    },
    {
      ...
      cell: ({row})=> <div className={cn("capitalize rounded-lg text-center p-2" , row.original.type === "income" && "bg-emerald-400/10 text-emerald-500" , row.original.type === "expense" && "bg-red-400/10 text-red-500")} >
          {row.original.type}
...
export default function TransactionTable({from, to}: Props) {
    ...
    const history = useQuery<GetTransactionsHistoryResponseType>({
        ...
        queryFn: async () => {
            const res = await fetch(`/api/transactions-history?from=${DateToUTCDate(from)}&to=${DateToUTCDate(to)}`)
            return res.json()
        }
     })
    ...
    return Array.from(uniqueCategories)
     ...
     return (
        ...
        <Skeletonwrap isLoading={history.isFetching}>
